...
export function AIChat() {
  ...
  const handleSend = () => {
    ...
    const assistantMsg: Message = {
      ...
      isStreaming: true,
      reasoning:
        "Let me analyze the user's question. I need to provide a comprehensive answer with relevant citations. I'll structure my response with clear sections and reference authoritative sources.",
      citations: SAMPLE_CITATIONS,
      branches: [
        "Here is the primary response to your question...",
        "An alternative perspective on this topic...",
      ],
      activeBranch: 0,
      feedback: null,
    }
...
According to the official documentation [1], React's component model enables building reusable UI. The App Router in Next.js [2] builds on this foundation with server components.
...
For type safety, TypeScript [3] adds static types that catch errors at compile time rather than runtime. This is especially valuable in larger codebases.
  ...
  const handleBranchSwitch = (msgId: string, direction: "prev" | "next") => {
    setMessages((prev) =>
      prev.map((m) => {
        ...
        const next =
          direction === "next"
            ? Math.min(current + 1, m.branches.length - 1)
                ...
                <div className="max-w-[85%] space-y-2">
                  ...
                  {msg.role === "assistant" && msg.reasoning && (
                    <button
                      ...
                      className="flex w-full items-center gap-1.5 rounded-md border border-dashed px-3 py-2 text-left text-xs text-muted-foreground transition-colors hover:bg-muted/50"
                    >
                      ...
                      {!expandedReasoning.has(msg.id) && (
                        <span className="truncate">— {msg.reasoning}</span>
                      )}
                    </button>
                  )}
                  {msg.role === "assistant" &&
                    ...
                    {msg.role === "assistant" && !msg.isStreaming && (
                      <>
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          className={`h-6 w-6 ${msg.feedback === "up" ? "text-emerald-500" : ""}`}
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          className={`h-6 w-6 ${msg.feedback === "down" ? "text-red-500" : ""}`}
                        ...
                        {msg.branches && msg.branches.length > 1 && (
                          <div className="ml-2 flex items-center gap-1 rounded border px-1">
                            <Button
                              ...
                              size="icon"
                              className="h-5 w-5"
                              disabled={(msg.activeBranch ?? 0) === 0}
                              onClick={() => handleBranchSwitch(msg.id, "prev")}
                            >
                              <ChevronLeft className="h-3 w-3" />
                            </Button>
                            <span className="text-xs text-muted-foreground">
                              {(msg.activeBranch ?? 0) + 1}/{msg.branches.length}
                            </span>
                            <Button
                              variant="ghost"
                              size="icon"
                              ...
                              onClick={() => handleBranchSwitch(msg.id, "next")}
                            >
        ...
        >
          ...
          <Button
            ...
            onClick={() => {
              ...
              simulateStreaming(
                newId,
                "Here's an alternative response. In a real app, each regeneration queries the AI model for a fresh answer, which can then be accessed via branch navigation."
              )
            }}
          >
